...
//                                   IS near the goal
//###################################################
bool Node3D::isNearGoal(const Node3D& goal) const {
...
//                                      MOVEMENT COST
//###################################################
void Node3D::updateG(double traver_value) {
  ...
  if (prim < 3) {
    ...
    if (pred->prim != prim) {
      ...
      if (pred->prim > 2) {
        g += dx[0] * Constants::penaltyTurning * Constants::penaltyCOD * (1-traver_value)*Constants::penalityTraver;
      } else {
        g += dx[0] * Constants::penaltyTurning * (1-traver_value)*Constants::penalityTraver;
      }
    } else {
      g += dx[0] * (1-traver_value)*Constants::penalityTraver;
    }
  }
  // reverse driving
  else {
    ...
    if (pred->prim != prim) {
      // penalize change of direction
      if (pred->prim < 3) {
        g += dx[0] * Constants::penaltyTurning * Constants::penaltyReversing * Constants::penaltyCOD * (1-traver_value)*Constants::penalityTraver;
      } else {
        g += dx[0] * Constants::penaltyTurning * Constants::penaltyReversing * (1-traver_value)*Constants::penalityTraver;
      }
    } else {
      g += dx[0] * Constants::penaltyReversing * (1-traver_value)*Constants::penalityTraver;
    }
  }
}
